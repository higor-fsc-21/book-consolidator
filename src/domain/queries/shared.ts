...
type RawBookSummary = PrismaBook & {
  chapters: { isRead: boolean; _count: { questions: number } }[];
  sessions: Pick<PrismaRevisionSession, "completedAt" | "score">[];
};
...
export function reviveBookSummary(raw: RawBookSummary): BookSummary {
  return {
    ...raw,
    importance: raw.importance as Importance,
    startDate: toDateOrNull(raw.startDate),
