...
type ScoredSessionBook = {
  sessions: Array<Pick<RevisionSession, "completedAt" | "score">>;
};

const scoredSessionsOf = (book: ScoredSessionBook) =>
  book.sessions.filter((s) => s.completedAt !== null && s.score !== null);

export const avgScore = (book: ScoredSessionBook): number | null => {
  const completed = scoredSessionsOf(book);
...
export const effectiveConsolidationState = (
  book: ScoredSessionBook & Pick<Book, "consolidationState">,
  now: Date = new Date(),
): ConsolidationState =>
  consolidationStateFor({
    current: book.consolidationState,
    completedSessions: scoredSessionsOf(book).map((s) => ({
      score: s.score ?? 0,
...
type ChapterBook = Pick<Book, "chapters"> | Pick<BookSummary, "chapters">;

export const readChapters = (book: ChapterBook) =>
  book.chapters.filter((c) => c.isRead).length;

export const readingProgress = (
  book: ChapterBook & Pick<Book, "totalChapters">,
): number =>
  book.totalChapters > 0 && readChapters(book) > 0
    ? Math.round((readChapters(book) / book.totalChapters) * 100)
    : 0;

export const totalQuestions = (book: ChapterBook) =>
  book.chapters.reduce(
    (total, chapter) =>
      total +
      ("questionCount" in chapter
